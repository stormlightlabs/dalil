...
function opengraphLogoDataUrl(svg: Buffer): string {
	const lightLogo = svg
		.toString()
		.replace(/<style>[\s\S]*?<\/style>/, '')
		.replace('class="mark"', 'fill="#f7f1f3"');
	return `data:image/svg+xml;base64,${Buffer.from(lightLogo).toString('base64')}`;
}
...
export async function GET(): Promise<Response> {
	const [googleSans, ibmPlexSans, favicon] = await Promise.all([
		...
		readFile(join(process.cwd(), 'src', 'lib', 'assets', 'favicon.svg')).then(opengraphLogoDataUrl)
	]);
